...
#
APPLICATION_NAME=${APPLICATION_NAME:-mem-public}
APPLICATION_ROUTE_NAME=${APPLICATION_ROUTE_NAME:-mmti-public-new}
APPLICATION_PORT=${APPLICATION_PORT:-8080-tcp}
STATIC_PAGE_NAME=${STATIC_PAGE_NAME:-mem-proxy-caddy}
STATIC_PAGE_PORT=${STATIC_PAGE_PORT:-2015-tcp}
#
IMG_SRC=${IMG_SRC:-bcgov-s2i-caddy}
GIT_REPO=${GIT_REPO:-https://github.com/bcgov/mem-mmti-public.git}
...
then
	oc process -f ${OC_DEPLOY} -n ${PROJECT} -p NAME=${STATIC_PAGE_NAME} BUILD_PROJECT=mem-mmt-tools \
		| oc apply -f -
	oc get route ${STATIC_PAGE_NAME} || \
		oc expose svc ${STATIC_PAGE_NAME}
	oc get dc ${STATIC_PAGE_NAME} -o json | grep '"image":' | awk '{ print $2 }' | tr -d ',"' \
